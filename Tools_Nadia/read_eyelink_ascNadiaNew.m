...
function [asc] = read_eyelink_ascNadiaNew(filename)
...
while ~feof(fid)
    ...
    elseif regexp(tline, '^MSG')
        ...
        elseif regexp(tline, '^MSG\s\d*\sTRIAL_VAR_LABELS 42')
            asc.secondretr = cat(1, asc.secondretr, {tline})
        elseif regexp(tline, '^MSG\s\d*\sTRIAL_VAR_LABELS 32')
            asc.secondretr = cat(1, asc.secondretr, {tline})
        elseif regexp(tline, '^MSG\s\d*\sTRIAL_VAR_LABELS 142')
            asc.secondretr = cat(1, asc.secondretr, {tline})
        elseif regexp(tline, '^MSG\s\d*\sTRIAL_VAR_LABELS 132')
            asc.secondretr = cat(1, asc.secondretr, {tline})
        elseif regexp(tline, '^MSG\s\d*\sTRIAL_VAR_LABELS 134')
            asc.secondretr = cat(1, asc.secondretr, {tline})
        end
        
        % for blockend
    elseif regexp(tline, '^END')
        asc.msg = cat(1, asc.msg, {tline});
    ...
    elseif regexp(tline, '^EBLINK')
       ...
       disp('not parsed') % all other lines are not parsed
    end
end
