function [data, event, blinksmp, saccsmp] = asc2dat_NP(asc)
...
for i=1:length(asc.msg)
    ...
    elseif ~isempty(regexp(asc.msg{i}, '^MSG\s\d*\sTRIAL_VAR_LABELS 42')) || ~isempty(regexp(asc.msg{i}, '^MSG\s\d*\sTRIAL_VAR_LABELS 32'))...
        ...
        %event(i+addrow).value = asc.msg{i+addrow}
        tmp = str2double(strtok{2})+800
        event(s+addrow).value = tmp;
        event(s+addrow).sample = dsearchn(asc.dat(1,:)', tmp)
        s=s+1;
    end
end
